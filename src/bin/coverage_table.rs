//! Generate a Markdown coverage table showing rblint cop coverage vs vendor RuboCop.
//!
//! Usage:
...
#[derive(Parser)]
#[command(about = "Generate rblint cop coverage table")]
struct Args {
...
fn render_conformance(project_root: &Path, cop_count: usize) -> Option<String> {
    ...
    writeln!(out).unwrap();
    writeln!(out, "Location-level comparison: file + line + cop_name. Only cops implemented by rblint ({cop_count}) are compared.").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "| Repo | rblint | rubocop | Matches | FP (rblint only) | FN (rubocop only) | Match rate |").unwrap();
    writeln!(out, "|------|-------:|--------:|--------:|-----------------:|------------------:|-----------:|").unwrap();

    for &repo in REPO_ORDER {
        if let Some(c) = data.get(repo) {
            writeln!(out, "| {repo} | {} | {} | {} | {} | {} | **{:.1}%** |",
                c.rblint_count, c.rubocop_count, c.matches,
...
fn main() {
    ...
    // 2. rblint cops per department from registry
    let registry = rblint::cop::registry::CopRegistry::default_registry();
    let mut rblint_cops: BTreeMap<String, BTreeSet<String>> = BTreeMap::new();
    for name in registry.names() {
        if let Some(slash) = name.find('/') {
            rblint_cops.entry(name[..slash].to_string()).or_default().insert(name.to_string());
        }
    }
    ...
    all_depts.extend(rblint_cops.keys().cloned());
    ...
    let mut out = String::new();
    writeln!(out, "# rblint Coverage Report").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "> Auto-generated by `cargo run --bin coverage_table`. Do not edit manually.").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "## Cop Coverage").unwrap();
    writeln!(out).unwrap();
    writeln!(out, "**rblint cops:** {}", registry.len()).unwrap();
    writeln!(out).unwrap();
    writeln!(out, "| Department | RuboCop | rblint | Coverage |").unwrap();
    writeln!(out, "|------------|--------:|-------:|---------:|").unwrap();
    ...
    for dept in &sorted_depts {
        ...
        let r = rblint_cops.get(dept).map_or(0, |s| s.len());
        total_v += v;
        total_r += r;
    ...
    if args.show_missing {
        ...
        for dept in &sorted_depts {
            if let Some(vs) = vendor_cops.get(dept) {
                let rs = rblint_cops.get(dept).cloned().unwrap_or_default();
        ...
        for dept in &sorted_depts {
            ...
            if let Some(rs) = rblint_cops.get(dept) {
                extras.extend(rs.difference(&vs).cloned());
            }
        }
        if !extras.is_empty() {
            writeln!(out).unwrap();
            writeln!(out, "### Extra Cops (in rblint, not in vendor)").unwrap();
            writeln!(out).unwrap();
    ...
    if let Some(conform) = render_conformance(&project_root, registry.len()) {
        ...
        writeln!(out, "## Conformance").unwrap();
        writeln!(out).unwrap();
        writeln!(out, "*No data. Run `cargo run --release --bin bench_rblint -- conform` to generate bench/conform.json.*").unwrap();
    }
