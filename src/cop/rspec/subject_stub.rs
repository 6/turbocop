...
use crate::cop::node_type::{BLOCK_NODE, CALL_NODE, DEF_NODE, LOCAL_VARIABLE_READ_NODE, STATEMENTS_NODE, SYMBOL_NODE};
...
fn contains_receive_call(node: &ruby_prism::Node<'_>) -> bool {
    if let Some(call) = node.as_call_node() {
        let name = call.name().as_slice();
        if (name == b"receive"
            || name == b"receive_messages"
            || name == b"receive_message_chain")
