...
#   covered-cops.txt    — list of rblint cops
...
unless bench_data.empty?
  md << "## Performance\n\n"
  md << "| Repo | .rb files | rblint (median) | rubocop (median) | Speedup |\n"
  md << "|------|-----------|-----------------|------------------|---------|\n"

  bench_data.each do |repo, data|
    results = data["results"]
    rblint_result = results.find { |r| r["command"] == "rblint" }
    rubocop_result = results.find { |r| r["command"] == "rubocop" }

    rblint_median = rblint_result&.dig("median") || 0
    ...
    speedup = format_speedup(rubocop_median, rblint_median)
    md << "| #{repo} | #{rb_str} | **#{format_time(rblint_median)}** | #{format_time(rubocop_median)} | **#{speedup}** |\n"
  end
...
unless conform_data.empty?
  md << "## Conformance\n\n"
  md << "Location-level comparison: file + line + cop_name. Only cops implemented by rblint are compared.\n\n"
  md << "| Repo | rblint | rubocop | Matches | FP (rblint only) | FN (rubocop only) | Match rate |\n"
  md << "|------|--------|---------|---------|------------------|-------------------|------------|\n"

  conform_data.each do |repo, data|
    md << "| #{repo} | #{data["rblint_count"]} | #{data["rubocop_count"]} | #{data["matches"]} | #{data["false_positives"]} | #{data["false_negatives"]} | **#{data["match_rate"]}%** |\n"
  end
