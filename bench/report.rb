...
#   covered-cops.txt    — list of nitrocop cops
...
unless bench_data.empty?
  md << "## Performance\n\n"
  md << "| Repo | .rb files | nitrocop (median) | rubocop (median) | Speedup |\n"
  md << "|------|-----------|-----------------|------------------|---------|\n"

  bench_data.each do |repo, data|
    results = data["results"]
    nitrocop_result = results.find { |r| r["command"] == "nitrocop" }
    rubocop_result = results.find { |r| r["command"] == "rubocop" }

    nitrocop_median = nitrocop_result&.dig("median") || 0
    ...
    speedup = format_speedup(rubocop_median, nitrocop_median)
    md << "| #{repo} | #{rb_str} | **#{format_time(nitrocop_median)}** | #{format_time(rubocop_median)} | **#{speedup}** |\n"
  end
...
unless conform_data.empty?
  md << "## Conformance\n\n"
  md << "Location-level comparison: file + line + cop_name. Only cops implemented by nitrocop are compared.\n\n"
  md << "| Repo | nitrocop | rubocop | Matches | FP (nitrocop only) | FN (rubocop only) | Match rate |\n"
  md << "|------|--------|---------|---------|------------------|-------------------|------------|\n"

  conform_data.each do |repo, data|
    md << "| #{repo} | #{data["nitrocop_count"]} | #{data["rubocop_count"]} | #{data["matches"]} | #{data["false_positives"]} | #{data["false_negatives"]} | **#{data["match_rate"]}%** |\n"
  end
