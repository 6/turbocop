...
- **NEVER copy code from private repos into this codebase.** When fixing false positives found by running against private/internal repos, write generic test cases that reproduce the same pattern. Use generic names (e.g. `records`, `payload`, `User`) instead of domain-specific names from the private codebase. Do not reference private repo names or paths in committed files. This applies to test fixtures, comments, and any other checked-in files.
