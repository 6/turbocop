...
- After adding a new cop, ensure `cargo test` passes — the `all_cops_have_minimum_test_coverage` integration test enforces that every cop has at least 3 offense fixture cases and 5+ non-empty lines in no_offense.rb. If a cop inherently has fewer patterns, add it to the exemption list in `tests/integration.rs` with a comment explaining why.
